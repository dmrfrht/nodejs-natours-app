...
exports.login = catchAsync(
  async (req, res, next) => {
    ...
    // 2) check if exists && password is correct
...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) get token and check of its exists
  let token
  ...
  if (!currenthUser) next(new AppError('the user belonging to this token does no longer exists', 401))

  // 4) check if user changed password after the token was issued
  if (currenthUser.changedPasswordAfter(decoded.iat)) next(new AppError('user recently changed password! please log in again', 401))
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) next(AppError('your current password is wrong', 401))
